...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config) (*ProcessResult, error) {
	...
	// Finalize the block first – consensus engine may mutate balances.
	...
	// NOTE: We intentionally no longer flush the REVM overlay here. Doing so
	// during block verification mutates the underlying trie and leads to
	// state-root mismatches against blocks generated by the legacy Go-EVM
	// (e.g. in tests that use BlockGen). Read-only callers that need to see
	// intermediate state should invoke revmbridge.FlushPendingFor themselves
	// after execution.
